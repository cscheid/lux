...
function equal_eps(x, y, eps)
{
    if (!eps) eps = vec.eps;
    return Math.abs(x - y) < eps;
};

function less_eps(x, y)
{
    return (x - y) < vec.eps;
};
...
test("vec properties", function() {
    ...
    _.each([2, 3, 4], function (d) {
        ...
        var s = Math.random();
        ok(t.dot(v1, v1) > 0);
        ok(less_eps(t.dot(v1, v2) * t.dot(v1, v2), 
        ...
        ok(equal_eps(t.dot(v1, t.scaling(v2, s)),
                     t.dot(t.scaling(v1, s), v2)), "dot scale");
        ok(equal_eps(t.dot(v1, t.scaling(v2, s)), 
                     s * t.dot(v1, v2)), "dot scale");
        ok(equal_eps(t.length(t.scaling(v1, s)),
        ...
        ok(t.equal_eps(t.schur_product(v1, v2),
                       t.schur_product(v2, v1)));
        ok(t.equal_eps(t.schur_product(v1, t.schur_product(v2, v3)),
                       t.schur_product(v2, t.schur_product(v3, v1))));
        ok(t.equal_eps(t.scaling(t.normalized(v1), t.length(v1)), v1));
        var e1 = t.create(), e2 = t.create();
        var i = Math.floor(Math.random() * d);
        e1[i] = Math.random();
        do {
            e2[i] = Math.random();
        } while (equal_eps(e1[i], e2[i]));
        ok(!equal_eps(e1[i], e2[i]));
    });

    var v1 = vec3.random(), v2 = vec3.random(), v3 = vec3.random();
    ok(vec3.equal(vec3.cross(v1, v2),
                  vec3.negative(vec3.cross(v2, v1))));
    ok(equal_eps(vec3.dot(v1, vec3.cross(v2, v3)),
                 mat3.determinant(mat3.make([v1[0], v1[1], v1[2],
                                             v2[0], v2[1], v2[2],
                                             v3[0], v3[1], v3[2]]))));

    ok(vec3.equal_eps(vec3.scaling(v1, vec3.dot(v1, vec3.cross(v2, v3))),
...
test("mat4 properties", function() {
    ...
    ok(equal_eps(mat4.determinant(mat4.product(m1, m2)),
                 mat4.determinant(m1) * mat4.determinant(m2)));
    ok(equal_eps(mat4.determinant(mat4.rotation_of(m1, a, v)),
                 mat4.determinant(m1)));
    ok(equal_eps(mat4.determinant(mat4.rotation(a, v)), 1));
    var d = mat4.determinant(m2);
    mat4.rotate(m2, a, v);
    ok(equal_eps(mat4.determinant(m2), d));
    ok(equal_eps(mat4.determinant(m2) * v[0] * v[1] * v[2],
...
test("mat properties", function() {
    _.each([2, 3, 4], function (d) {
        ...
        var vt = vec[d];
        var v = vt.random();
        ok(equal_eps(mt.determinant(mt.product(m1, m2)),
        ...
        do {
            e2[i1] = Math.random();
        } while (equal_eps(e2[i1], e1[i1]));
        ok(!mt.equal(e1, e2));
        ok(less_eps(vt.length(mt.product_vec(m1, v)),
                    mt.frobenius_norm(m1) * vt.length(v)));
        ok(vt.equal_eps(mt.product_vec(mt.product(m1, m2), v),
                        mt.product_vec(m1, mt.product_vec(m2, v))));

        var det = mt.determinant(m1);

        // comparison more tolerant because this is numerically bad.
        ok(equal_eps(mt.frobenius_norm(mt.product(m1, mt.inverse(m1))), Math.sqrt(d),
                     Math.sqrt(vec.eps)));
    });
});
