...
function init(opts)
{
  ...
  try {
    ...
    if ("interactor" in opts) {
      ...
      for (key in opts.interactor.events) {
        if (opts[key]) {
    ...
    if (opts.debugging) {
      ...
      gl = Lux.Lib.WebGLDebugUtils.makeDebugContext(
        gl, throwOnGLError, opts.tracing);
    }
